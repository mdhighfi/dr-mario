(function () {
  ...
  Grid.prototype.isEmpty = function(gridPos) {
    var square = this.squares[gridPos[0]][gridPos[1]];
    var empty = Mario.Constants.EMPTY;
    return (square === empty);
  };
  ...
  Grid.prototype.furthestEmptyBelow = function(gridPos) {
    ...
    while ( grid.isEmpty(posToCheck) && (grid.inBounds(posToCheck)) ){
      posToCheck = util.moveOneDown(posToCheck);
    }
    return util.moveOneUp(posToCheck)
  };
})();
